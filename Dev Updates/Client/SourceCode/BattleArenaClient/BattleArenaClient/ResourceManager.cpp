...
void ResourceManager::LoadFontSprites(ID3D12Device* device, ID3D12CommandQueue* commandQueue)
{
    ...
    m_Fonts[L"맑은 고딕(16pt)"]
        = std::make_unique<DXTK_FONT>(device, commandQueue, additionalAssetPath + L"UI/Font/맑은 고딕(16pt).spritefont");
    m_Fonts[L"맑은 고딕(12pt)"]
        = std::make_unique<DXTK_FONT>(device, commandQueue, additionalAssetPath + L"UI/Font/맑은 고딕(12pt).spritefont");
}
