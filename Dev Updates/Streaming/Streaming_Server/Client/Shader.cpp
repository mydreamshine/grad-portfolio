...
void CShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dRootSignature, bool Wired, int Index)
{
	ID3DBlob *pd3dVertexShaderBlob = NULL, *pd3dPixelShaderBlob = NULL;
	D3D12_GRAPHICS_PIPELINE_STATE_DESC d3dPipelineStateDesc;
	...
	d3dPipelineStateDesc.SampleDesc.Count = 1;
	d3dPipelineStateDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
	pd3dDevice->CreateGraphicsPipelineState(&d3dPipelineStateDesc, __uuidof(ID3D12PipelineState), (void **)&m_ppd3dPipelineStates[Index]);
...
void CUIShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState*[m_nPipelineStates];
	CShader::CreateShader(pd3dDevice, pd3dRootSignature);
}
void CUIShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera, D3D12_CPU_DESCRIPTOR_HANDLE *pHandle, int life) {
	CShader::Render(pd3dCommandList, pCamera);
	//UI는 항상 그려져야 한다
	pd3dCommandList->ClearDepthStencilView(*pHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
	pd3dCommandList->DrawInstanced(3, life, 0, 0);
}
// ========================================================================
...
void CObjectsShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState*[m_nPipelineStates];
	CShader::CreateShader(pd3dDevice, pd3dGraphicsRootSignature, false, 0);
}
...
CInstancingShader::CInstancingShader(CGameObject** pObjects, int nObjects, CMesh** pMeshs, int nMeshs)
{
...
void CInstancingShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	...
	m_ppd3dPipelineStates = new ID3D12PipelineState*[m_nPipelineStates];
	CShader::CreateShader(pd3dDevice, pd3dGraphicsRootSignature, false, 0);
	CShader::CreateShader(pd3dDevice, pd3dGraphicsRootSignature, true, 1);
}
...
void CInstancingShader::UpdateShaderVariables(ID3D12GraphicsCommandList	*pd3dCommandList)
{
	...
	{
		m_pcbMappedGameObjects[i].m_xmcColor = m_ppObjects[i]->GetColor();
		XMStoreFloat4x4(&m_pcbMappedGameObjects[i].m_xmf4x4Transform, XMMatrixTranspose(XMLoadFloat4x4(&m_ppObjects[i]->m_xmf4x4World)));
	}
}
void CInstancingShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera	*pCamera, D3D12_CPU_DESCRIPTOR_HANDLE* pHandle, bool state)
{
	CShader::Render(pd3dCommandList, pCamera);
	UpdateShaderVariables(pd3dCommandList);
	pd3dCommandList->IASetVertexBuffers(1, 1, &m_d3dInstancingBufferView);

	for (int i = 2; i < m_nMeshs; ++i)
		m_ppMeshs[i]->Render(pd3dCommandList);

	//플레이어가 충돌하면 WireFrame으로 그리게 함
	if (state) {
		pd3dCommandList->SetPipelineState(m_ppd3dPipelineStates[1]);
		pd3dCommandList->ClearDepthStencilView(*pHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
	}
	m_ppMeshs[0]->Render(pd3dCommandList);
	m_ppMeshs[1]->Render(pd3dCommandList);
}